...
<?php
/**
 * Loom-patched Typesense DB Indexer
 *
 * Identical to config/tribe/index_db.php with one addition:
 *   TYPESENSE_COLLECTION_PREFIX env var scopes all collections to a Thread.
 *   e.g. prefix "acme" → collection "acme_user" instead of "tribe_user".
 *
 * Modes
 * ─────
 *   Full re-index:          php index_db.php
 *   Single upsert:          php index_db.php <id>
 *   Single delete:          php index_db.php <id> delete
 */
...
if (file_exists($initFile)) {
    ...
    echo '[' . date('Y-m-d H:i:s') . '] WARN: _init.php not found — running in standalone PDO mode.' . PHP_EOL;
}
...
if (file_exists($autoload)) {
    ...
    echo '[' . date('Y-m-d H:i:s') . '] FATAL: vendor/autoload.php not found.' . PHP_EOL;
    exit(1);
}
...
// ── LOOM ADDITION: per-thread collection prefix ───────────────────────────────
$TS_PREFIX = getenv('TYPESENSE_COLLECTION_PREFIX') ?: ($_ENV['TYPESENSE_COLLECTION_PREFIX'] ?? '');
...
/**
 * LOOM PATCH: prefix scopes collections to this Thread.
 * With prefix "acme": collection_name("user") → "acme_user"
 * Without prefix:     collection_name("user") → "tribe_user"  (original behaviour)
 */
function collection_name(string $type): string
{
    global $TS_PREFIX;
    return $TS_PREFIX ? "{$TS_PREFIX}_{$type}" : "tribe_{$type}";
}
...
function ensure_collection(TsClient $ts, string $type, string $collName, ?object $tribeTs): void
{
    try {
...
function parse_import_response($raw): array
{
    ...
    if (is_array($raw)) {
        foreach ($raw as $line) {
            if ($line['success'] ?? false) $ok++;
            else { $fail++; $failMsgs[] = $line['error'] ?? '?'; }
        }
    } else {
...
function wait_for_typesense(TsClient $ts, int $maxWait = 60): void
{
    ...
    while ($waited < $maxWait) {
        try { $ts->health->retrieve(3); log_msg("Typesense is ready."); return; }
        catch (\Exception $e) { log_msg("Typesense not ready ({$waited}s)…"); sleep(3); $waited += 3; }
    }
...
if ($tribeBootstrapped) {
    try { $tribeTs = new \Tribe\Typesense(); }
    catch (\Exception $e) { log_msg("WARN: Could not instantiate Tribe\\Typesense: " . $e->getMessage()); }
}
...
if ($TS_PREFIX) {
    log_msg("Collection prefix: '{$TS_PREFIX}' (Loom Thread isolation)");
}
...
if ($singleId !== null) {
    ...
    if ($mode === 'delete') {
        ...
        if (empty($rows)) { log_msg("Object {$id} not found in DB."); exit(0); }
        $obj  = hydrate_rows($rows)[0];
        $type = $obj['type'] ?? '';

        if (!$type || in_array($type, SKIP_TYPES)) { log_msg("Skipping delete for type '{$type}': {$id}"); exit(0); }
    ...
    $rows = db_query("SELECT `id`, `type`, `content`, `updated_on`, `created_on` FROM `data` WHERE `id`={$id} LIMIT 1", $tribeDb);
    if (empty($rows)) { log_msg("Object {$id} not found in DB."); exit(1); }

    $obj  = hydrate_rows($rows)[0];
    $type = $obj['type'] ?? '';

    if (!$type || in_array($type, SKIP_TYPES)) { log_msg("Skipping type '{$type}': {$id}"); exit(0); }
...
do {
    ...
    foreach ($byType as $type => $docs) {
        ...
        foreach (array_chunk($docs, TS_BATCH) as $batch) {
            try {
                $raw    = $tsClient->collections[$collName]->documents->import($batch, ['action' => 'upsert']);
                $parsed = parse_import_response($raw);

                $stats[$type]['ok']   = ($stats[$type]['ok']   ?? 0) + $parsed['ok'];
                $stats[$type]['fail'] = ($stats[$type]['fail'] ?? 0) + $parsed['fail'];

                foreach ($parsed['errors'] as $em) { log_msg("  WARN [{$type}] {$em}"); }
            } catch (\Exception $e) {
