...
yes | certbot --agree-tos --no-eff-email --email admin_email --nginx -d xyz.com;
...
yes | mysql -umysql_w_user -pmysql_w_pass mysql_w_user < install_path/xyz.com/install/install.sql;

rm install_path/xyz.com/install -R;
rm install_path/xyz.com/.env.sample;
chown www-data: install_path/xyz.com/uploads -R;
yes | composer create-project phpmyadmin/phpmyadmin install_path/xyz.com/phpmyadmin;
yes | composer update;
